...
uint8_t bzrtpUtils_getAllAvailableCryptoTypes(uint8_t algoType,
  ...
  case ZRTP_KEYAGREEMENT_TYPE: {
    ...
    /* Put them in a list in a prefered order, if no configuration is done the
     * first 6 are used */
    uint8_t index = 0;
    ...
    if ((available_key_agreements & BCTBX_KEM_X25519) &&
        (available_key_agreements & BCTBX_KEM_MLKEM512)) {
      availableTypes[index] = ZRTP_KEYAGREEMENT_K255_MLK512;
      index++;
    }

    if ((available_key_agreements & BCTBX_KEM_X25519) &&
        (available_key_agreements & BCTBX_KEM_KYBER512)) {
      availableTypes[index] = ZRTP_KEYAGREEMENT_K255_KYB512;
      index++;
    }

    if ((available_key_agreements & BCTBX_KEM_X25519) &&
        (available_key_agreements & BCTBX_KEM_HQC128)) {
      availableTypes[index] = ZRTP_KEYAGREEMENT_K255_HQC128;
      index++;
    }

    if ((available_key_agreements & BCTBX_KEM_X448) &&
        (available_key_agreements & BCTBX_KEM_MLKEM1024)) {
      availableTypes[index] = ZRTP_KEYAGREEMENT_K448_MLK1024;
      index++;
    }

    if ((available_key_agreements & BCTBX_KEM_X448) &&
        (available_key_agreements & BCTBX_KEM_KYBER1024)) {
      availableTypes[index] = ZRTP_KEYAGREEMENT_K448_KYB1024;
      index++;
    }

    if ((available_key_agreements & BCTBX_KEM_X448) &&
        (available_key_agreements & BCTBX_KEM_HQC256)) {
  ...
  case ZRTP_SAS_TYPE: /* the SAS function is implemented in cryptoUtils.c and
                         then is not directly linked to the polarSSL crypto
                         wrapper */
...
uint8_t bzrtpUtils_getAvailableCryptoTypes(uint8_t algoType,
  ...
  if (algoType != ZRTP_KEYAGREEMENT_TYPE) {
    ...
    if (ret < 7) {
      ...
      /* buggy behavior, we need this one for retrocompatibility, enforce its
       * presence at the end of the list */
...
uint8_t bzrtpUtils_getMandatoryCryptoTypes(uint8_t algoType,
  ...
  case ZRTP_KEYAGREEMENT_TYPE:
    ...
    mandatoryTypes[1] =
        ZRTP_KEYAGREEMENT_Mult; /* we must add this one if we want to be able to
                                   make multistream */
    return 2;
  case ZRTP_SAS_TYPE:
...
int bzrtp_keyDerivationFunction(const uint8_t *key, const size_t keyLength,
                                ...
                                void (*hmacFunction)(const uint8_t *, size_t,
                                                     const uint8_t *, size_t,
                                                     uint8_t, uint8_t *),
                                uint8_t *output) {

  /* get the total length (in bytes) of the data to be hashed */
  /* need to add 4 bytes for the initial constant 0x00000001, 1 byte for the
   * 0x00 separator and 4 bytes for the hmacLength length */
  ...
  /* fill the input starting by the 32-bits big-endian interger set to
   * 0x00000001 */
  size_t index = 0;
  input[index++] = 0x00;
  input[index++] = 0x00;
  ...
  /* end by L(hmacLength) in big-endian. hmacLength is in bytes and must be
   * converted to bits before insertion in the text to hash */
  input[index++] =
      0; //(uint8_t)((hmacLength>>21)&0xFF); - hmacLength in bit cannot be more
         // than 2^11 so the first 2 bytes are always 0
...
void bzrtp_base256(uint32_t sas, char *output, int outputSize) {
  ...
  if (outputSize == 32) {
    ...
    snprintf(output, outputSize, "%s:%s", pgpWordsEven[evenIndex],
             pgpWordsOdd[oddIndex]);
  } else {
    snprintf(output, outputSize, "%s", pgpWordsEven[evenIndex]);
  }
}

void bzrtp_generate_incorrect_sas(uint32_t sas, char **incorrectSas,
                                  uint8_t sasAlgo) {
  int nbUsedBits;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    do {
      ...
      for (int j = 0; j < i; j++) {
        if (((randIntTab[j] >> (32 - nbUsedBits)) & 0x3ff) ==
            ((randInt >> (32 - nbUsedBits)) & 0x3ff)) {
          sameAsOtherIncorrectSas = TRUE;
        }
      }
    } while ((((randInt >> (32 - nbUsedBits)) & 0x3ff) == sasFirstPart) ||
             (((randInt >> (32 - nbUsedBits)) & 0x3ff) == sasSecondPart) ||
             sameAsOtherIncorrectSas);
...
uint32_t bzrtp_CRC32(uint8_t *input, uint16_t length) {
  ...
  int)CRC32LookupTable[i], (long unsigned int)CRC32LookupTable[i+1], (long
  unsigned int)CRC32LookupTable[i+2], (long unsigned int)CRC32LookupTable[i+3]);
  }*/
  ...
  /* swap the byte order */
  return ((crc & 0xFF000000) >> 24) | ((crc & 0x00FF0000) >> 8) |
...
/*
 * @brief select a key agreement algorithm from the one available in context and
 * the one provided by peer in Hello Message as described in rfc section 4.1.2
 * - other algorithm are selected according to availability and selected key
 * agreement as described in rfc section 5.1.5 The other algorithm choice will
 * finally be set by the endpoint acting as initiator in the commit packet
 *
 * @param[in,out]	zrtpContext			The context contains the
 * list of available algo and is set with the selected ones and associated
 * functions
 * @param[in]		peerHelloMessage	The peer hello message
 * containing his set of available algos
 *
...
int bzrtp_cryptoAlgoAgreement(bzrtpContext_t *zrtpContext,
  ...
  commonKeyAgreementTypeNumber = bzrtp_selectCommonAlgo(
      zrtpContext->supportedKeyAgreement, zrtpContext->kc,
      peerHelloMessage->supportedKeyAgreement, peerHelloMessage->kc,
      selfCommonKeyAgreementType);

  /* check we have something in common */
  if (commonKeyAgreementTypeNumber ==
      0) { /* this shall never bee true as all ZRTP endpoint MUST support at
              least DH3k */
    return ZRTP_CRYPTOAGREEMENT_NOCOMMONALGOFOUND;
  }

  /* peer ordering: get the common list in the peer order of preference */
  bzrtp_selectCommonAlgo(peerHelloMessage->supportedKeyAgreement,
                         peerHelloMessage->kc,
                         zrtpContext->supportedKeyAgreement, zrtpContext->kc,
                         peerCommonKeyAgreementType);
  ...
  } else { /* select the fastest of the two algoritm. Order is "DH2k", "E255",
              "EC25", "E448", "DH3k", "EC38", "EC52" is defined by value order
              from bzrtp.h */
    if (peerCommonKeyAgreementType[0] <
        selfCommonKeyAgreementType[0]) { /* mapping to uint8_t is defined to
                                            have them in the fast to slow order
                                          */
  ...
  /* now we shall select others algos among the availables and set them into the
   * context, these choices may be bypassed if we assume the receptor role as
   * the initiator's commit will have the final word on the algo selection */

  /*** Cipher block algorithm ***/
  /* get the self cipher types availables */
  commonCipherTypeNumber =
      bzrtp_selectCommonAlgo(zrtpContext->supportedCipher, zrtpContext->cc,
                             peerHelloMessage->supportedCipher,
                             peerHelloMessage->cc, commonCipherType);

  if (commonCipherTypeNumber == 0) { /* This shall never happend but... */
    return ZRTP_CRYPTOAGREEMENT_INVALIDCIPHER;
  }
  /* rfc section 5.1.5 specifies that if EC38 is choosen we SHOULD use AES256 or
   * AES192 */
  /* if a post quantum algorithm is choosen we SHOULD use AES256 */
  if (zrtpChannelContext->keyAgreementAlgo == ZRTP_KEYAGREEMENT_EC38 ||
      bzrtp_isPostQuantum(zrtpChannelContext->keyAgreementAlgo) ||
      zrtpChannelContext->keyAgreementAlgo == ZRTP_KEYAGREEMENT_X448 ||
      zrtpChannelContext->keyAgreementAlgo == ZRTP_KEYAGREEMENT_K448) {
    int i = 0;

    zrtpChannelContext->cipherAlgo = ZRTP_UNSET_ALGO;
    /* is AES3 available */
    while (i < commonCipherTypeNumber &&
           zrtpChannelContext->cipherAlgo == ZRTP_UNSET_ALGO) {
  ...
  commonHashTypeNumber = bzrtp_selectCommonAlgo(
      zrtpContext->supportedHash, zrtpContext->hc,
      peerHelloMessage->supportedHash, peerHelloMessage->hc, commonHashType);
   ...
   */
  /* if a post quantum algorithm or X448 is choosen we SHOULD use SHA512 (or
   * SHA384) */
  if (zrtpChannelContext->keyAgreementAlgo == ZRTP_KEYAGREEMENT_EC38 ||
      bzrtp_isPostQuantum(zrtpChannelContext->keyAgreementAlgo) ||
      zrtpChannelContext->keyAgreementAlgo == ZRTP_KEYAGREEMENT_X448 ||
  ...
  commonAuthTagTypeNumber =
      bzrtp_selectCommonAlgo(zrtpContext->supportedAuthTag, zrtpContext->ac,
                             peerHelloMessage->supportedAuthTag,
                             peerHelloMessage->ac, commonAuthTagType);
  ...
  commonSasTypeNumber = bzrtp_selectCommonAlgo(
      zrtpContext->supportedSas, zrtpContext->sc,
      peerHelloMessage->supportedSas, peerHelloMessage->sc, commonSasType);
...
/*
 * @brief Update context crypto function pointer according to related values of
 * choosen algorithms fields (hashAlgo, cipherAlgo, etc..) The associated length
 * are updated too
 *
 ...
 * @return			0 on succes
 */
int bzrtp_updateCryptoFunctionPointers(
    bzrtpChannelContext_t *zrtpChannelContext) {
  ...
  /* Key agreement algo : is directly managed in the parse and state machine,
   * nothing to do, just check it is valid */
...
#define BITMASK_256_SET_ZERO(bitmask)                                          \
  memset(bitmask, 0, sizeof(int) * BITMASK_256_SIZE)
#define BITMASK_256_SET(bitmask, value)                                        \
  bitmask[value / BITS_PRO_INT] |= 1 << (value % BITS_PRO_INT)
#define BITMASK_256_UNSET(bitmask, value)                                      \
  bitmask[value / BITS_PRO_INT] &= ~(1 << (value % BITS_PRO_INT))
#define BITMASK_256_CHECK(bitmask, value)                                      \
  (bitmask[value / BITS_PRO_INT] & 1 << (value % BITS_PRO_INT))

/**
 * @brief Select common algorithm from the given array where algo are
 * represented by their 4 chars string defined in rfc section 5.1.2 to 5.1.6
 * Master array is the one given the preference order
 * All algo are designed by their uint8_t mapped values
 *
 * @param[in]	masterArray	 		The ordered available algo,
 * result will follow this ordering
 * @param[in]	masterArrayLength	Number of valids element in the master
 * array
 * @param[in]	slaveArray	 		The available algo, order is not
 * taken in account
 * @param[in]	slaveArrayLength	Number of valids element in the slave
 * array
 * @param[out]	commonArray	 		Common algorithms found, max
 * size 7
 *
 * @return		the number of common algorithms found
 */
uint8_t bzrtp_selectCommonAlgo(uint8_t masterArray[7],
                               uint8_t masterArrayLength, uint8_t *slaveArray,
 ...
 * - SAS
 *
 * @param[in]		algoType		mapped to defines, must be in
 * [ZRTP_HASH_TYPE, ZRTP_CIPHERBLOCK_TYPE, ZRTP_AUTHTAG_TYPE,
 * ZRTP_KEYAGREEMENT_TYPE or ZRTP_SAS_TYPE]
 * @param[in,out]	algoTypes		mapped to uint8_t value of the 4
 * char strings giving the algo types as string according to rfc section 5.1.2
 * to 5.1.6
...
void bzrtp_addMandatoryCryptoTypesIfNeeded(uint8_t algoType,
  ...
  uint8_t mandatoryTypes[7];
  const uint8_t mandatoryTypesCount =
      bzrtpUtils_getMandatoryCryptoTypes(algoType, mandatoryTypes);
  ...
  for (i = 0, j = 0; j < 7 && (i < *algoTypesCount); i++) {
     ...
     */
    if ((j + missingTypesCount < 7 ||
         BITMASK_256_CHECK(missingBitmask, algoTypes[i])) &&
        !BITMASK_256_CHECK(algosBitmask, algoTypes[i])) {
...
/*
 * @brief Map the string description of algo type to an int defined in
 * cryptoWrapper.h
 *
 * @param[in] algoType		A 4 chars string containing the algo type as
 * listed in rfc sections 5.1.2 to 5.1.6
 * @param[in] algoFamily	The integer mapped algo family (ZRTP_HASH_TYPE,
 * ZRTP_CIPHERBLOCK_TYPE, ZRTP_AUTHTAG_TYPE, ZRTP_KEYAGREEMENT_TYPE or
 * ZRTP_SAS_TYPE)
 *
...
uint8_t bzrtp_cryptoAlgoTypeStringToInt(uint8_t algoType[4],
  ...
  case ZRTP_KEYAGREEMENT_TYPE: {
    if (memcmp(algoType, "DH3k", 4) == 0) {
      ...
      // hybrids Current HQC implementation is the one from round 4 submission
...
/*
 * @brief Unmap the string description of algo type to an int defined in
 * cryptoWrapper.h
 *
 * @param[in] algoTypeInt	The integer algo type defined in crypoWrapper.h
 * @param[in] algoFamily	The string code for the algorithm as defined in
 * rfc 5.1.2 to 5.1.6
 */
void bzrtp_cryptoAlgoTypeIntToString(uint8_t algoTypeInt,
  ...
  // Current HQC implementation is the one from round 4 submission (2024/02/23),
...
void bzrtp_DestroyKey(uint8_t *key, size_t keyLength, void *rngContext) {
  if (key != NULL) {
    bctbx_rng_get(static_cast<bctbx_rng_context_t *>(rngContext), key,
                  keyLength);
  }
}
...
void bzrtp_destroyKeyMaterial(bzrtpContext_t *zrtpContext,
  ...
  bzrtp_DestroyKey(zrtpChannelContext->KDFContext,
                   ...
                   zrtpContext->RNGContext);
  bzrtp_DestroyKey(zrtpChannelContext->mackeyi, zrtpChannelContext->hashLength,
                   zrtpContext->RNGContext);
  bzrtp_DestroyKey(zrtpChannelContext->mackeyr, zrtpChannelContext->hashLength,
                   zrtpContext->RNGContext);
...
static uint16_t bzrtp_getKEMPublicKeyLength(uint8_t keyAgreementAlgo) {
  ...
  case ZRTP_KEYAGREEMENT_K255_KYB512_HQC128:
    return bctoolbox::K25519::kPkSize + bctoolbox::KYBER512::kPkSize +
           bctoolbox::HQC128::kPkSize;
  case ZRTP_KEYAGREEMENT_K448_KYB1024_HQC256:
    return bctoolbox::K448::kPkSize + bctoolbox::KYBER1024::kPkSize +
           bctoolbox::HQC256::kPkSize;
  default:
    return 0;
  }
#else  /* HAVE_BCTBXPQ */
...
static uint16_t bzrtp_getKEMCipherTextLength(uint8_t keyAgreementAlgo) {
  ...
  case ZRTP_KEYAGREEMENT_K255_KYB512_HQC128:
    return bctoolbox::K25519::kCtSize + bctoolbox::KYBER512::kCtSize +
           bctoolbox::HQC128::kCtSize;
  case ZRTP_KEYAGREEMENT_K448_KYB1024_HQC256:
    return bctoolbox::K448::kCtSize + bctoolbox::KYBER1024::kCtSize +
           bctoolbox::HQC256::kCtSize;
  default:
    return 0;
  }
#else  /* HAVE_BCTBXPQ */
...
uint16_t bzrtp_computeKeyAgreementPublicValueLength(uint8_t keyAgreementAlgo,
  ...
  default:
    ...
    case MSGTYPE_COMMIT: // Commit packet includes the public key
    {
      ...
      if (ret % 4) { // The size MUST be a multiple of 4 as ZRTP nessage size is
                     // given in 4 bytes words not in bytes, it will just be
                     // padded at the end with 0 if needed
        ret += 4 - ret % 4;
      }
      return ret;
    }
    ...
    case MSGTYPE_DHPART2: // DHPart2 holds a nonce - invalid algo would pass if
                          // giving this message type
...
uint16_t bzrtp_computeCommitMessageVariableLength(uint8_t keyAgreementAlgo) {
  if (keyAgreementAlgo == ZRTP_KEYAGREEMENT_Prsh)
    return 24; /* nonce (16 bytes) and keyID(8 bytes) are 24 bytes length in
                  preshared Commit message format */
  if (keyAgreementAlgo == ZRTP_KEYAGREEMENT_Mult)
    return 16; /* nonce is 16 bytes length in multistream Commit message format
                */
 ...
 *
 * @param[in]	keyAgreementAlgo one of ZRTP_KEYAGREEMENT_KYB1,
 *ZRTP_KEYAGREEMENT_KYB2, ZRTP_KEYAGREEMENT_KYB3, ZRTP_KEYAGREEMENT_HQC1,
 *ZRTP_KEYAGREEMENT_HQC2, ZRTP_KEYAGREEMENT_HQC3, ZRTP_KEYAGREEMENT_K255,
 *ZRTP_KEYAGREEMENT_K448, ZRTP_KEYAGREEMENT_K255_KYB512,
 *ZRTP_KEYAGREEMENT_K255_HQC128, ZRTP_KEYAGREEMENT_K448_KYB1024,
...
bzrtp_KEMContext_t *bzrtp_createKEMContext(uint8_t keyAgreementAlgo,
  ...
  case ZRTP_KEYAGREEMENT_K448:
    ...
    break;
  case ZRTP_KEYAGREEMENT_K255_MLK512:
    context->ctx = std::make_shared<bctoolbox::HYBRID_KEM>(
        std::list<std::shared_ptr<bctoolbox::KEM>>(
            {std::make_shared<bctoolbox::K25519>(hashId),
             std::make_shared<bctoolbox::MLKEM512>()}),
        hashId);
    break;
  case ZRTP_KEYAGREEMENT_K255_KYB512:
    context->ctx = std::make_shared<bctoolbox::HYBRID_KEM>(
        std::list<std::shared_ptr<bctoolbox::KEM>>(
            {std::make_shared<bctoolbox::K25519>(hashId),
             std::make_shared<bctoolbox::KYBER512>()}),
        hashId);
    break;
  case ZRTP_KEYAGREEMENT_K255_HQC128:
    context->ctx = std::make_shared<bctoolbox::HYBRID_KEM>(
        std::list<std::shared_ptr<bctoolbox::KEM>>(
            {std::make_shared<bctoolbox::K25519>(hashId),
             std::make_shared<bctoolbox::HQC128>()}),
        hashId);
    break;
  case ZRTP_KEYAGREEMENT_K448_MLK1024:
    context->ctx = std::make_shared<bctoolbox::HYBRID_KEM>(
        std::list<std::shared_ptr<bctoolbox::KEM>>(
            {std::make_shared<bctoolbox::K448>(hashId),
             std::make_shared<bctoolbox::MLKEM1024>()}),
        hashId);
    break;
  case ZRTP_KEYAGREEMENT_K448_KYB1024:
    context->ctx = std::make_shared<bctoolbox::HYBRID_KEM>(
        std::list<std::shared_ptr<bctoolbox::KEM>>(
            {std::make_shared<bctoolbox::K448>(hashId),
             std::make_shared<bctoolbox::KYBER1024>()}),
        hashId);
    break;
  case ZRTP_KEYAGREEMENT_K448_HQC256:
    context->ctx = std::make_shared<bctoolbox::HYBRID_KEM>(
        std::list<std::shared_ptr<bctoolbox::KEM>>(
            {std::make_shared<bctoolbox::K448>(hashId),
             std::make_shared<bctoolbox::HQC256>()}),
        hashId);
    break;
  case ZRTP_KEYAGREEMENT_K255_KYB512_HQC128:
    context->ctx = std::make_shared<bctoolbox::HYBRID_KEM>(
...
int bzrtp_KEM_getSharedSecret(bzrtp_KEMContext_t *ctx, uint8_t *sharedSecret) {
  ...
  if (ctx->sharedSecret.size() > 0) {
    std::memcpy(sharedSecret, ctx->sharedSecret.data(),
                ctx->sharedSecret.size());
    return 0;
  }
  return BZRTP_ERROR_CONTEXTNOTREADY;
}
...
/**
 * Encapsulate the public key and store the cipher text in the context
 * @param[in]	ctx			a valid KEM context
 * @param[in]	publicKey	Bob의 공개키
 * @param[out]	cipherText	결과 ciphertext를 써 넣을 버퍼
 *
 * @return 0 on success
 */

int bzrtp_KEM_encaps(bzrtp_KEMContext_t *ctx, uint8_t *publicKey,
                     uint8_t *cipherText) {
  // 컨텍스트 검증
  if (ctx == NULL || ctx->ctx == NULL) {
    return BZRTP_ERROR_CONTEXTNOTREADY;
...
#else /* HAVE_BCTBXPQ */
/* KEM functions are implemented in postquantumcryptoengine, stub them if we
 * don't have it */
